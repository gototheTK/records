...
#### 질의문들로 이루어진 논리적 작업이다. BEGIN같이 명시적으로 선언할 수도 직접 선언할 수도 있다. 사실 질의문자체가 트랜잭션의 일종이다. COMMIT이나 ROLLBACK으로 트랜잭션의 끝맺음을 해주어야 데이터베이스에 반영되거나 작업취소가 된다. 그 전까지는 명시적인 잠금이 아닌한 히스토리 로그에 처리결과가 저장된다. 그리고 AUTO-COMMIT의 여부에따라 명시적으로 끝맺음을 해주거나 해줄필요가 없다. 그래서 DB접속시 COMMIT을 해주지않아도 변경될 수 있다.

<br/>

### 7.2. 트랜잭션의 성질
#### 1. 원자성 : 트랜잭션의 작업은 절대로 부분처리완료가 될 수 가 없다.

#### 2. 일관성: 트랜잭션 처리 완료 후에도 데이터베이스의 무결성과 일관성을 보장해야한다.

#### 3. 고립성: 각 세션들의 트랜잭션을 동시 처리할때 서로에게 영향을 주지 않게 보여야한다.

#### 4. 지속성: 트랜잭션 처리완료 후 데이터베이스에 반영이 지속되어야 한다.

<br/>

### 7.3. 트랜잭션의 동시성 문제와 잠금

#### 동시성 문제 : 각 세션의 트랜잭션의 동시 처리시 발생하는 처리결과 기준 선택 문제

#### 잠금 : 동시성 문제를 해결하기 위한 방법으로, 자바의 임계성 문제와같이 세션이 데이터에 접근시 가지는 권한을 일컫는다. 잠금을 명시적으로 믁시적으로 하느냐, 읽기만하느냐 쓰기도 하느냐, 테이블이냐 레코드냐에 따라 잠금의 종류는 달라진다.

#### 명시적잠금과 묵시적잠금 : 명시적 잠그은 FOR SHARE, FOR WRITE,LOCK TABLE ... READ, LOCK TABLE ... WRITE같이 덧붙이거나 직접 선언 할 수 있는 반면에, 묵시적 잠금은 UPDATE나 DELETE, INSERT같이 읽고 쓸때 질의처리기가 암묵적으로 선언해주는 잠금이다.

#### 공유적 잠금과 베타적 잠금 : 읽을 데이터의 일관성을 위한 공유적 잠금과 수정,입력,삭제시 접근을 제한하는 베타적 잠금이 있다. 공유적 잠금은 같은 공유적 잠금이면 읽을 수 있지만, 베타적 잠금은 같은 베타적 잠금이여도 대기하여야한다. 공유적 잠금은 FOR SHARE 혹은 TABLE ... READ로, 베타적 잠금은 FOR WRITE 혹은 TABLE ... WRITE로 선언할 수 있다. 공유적 잠금은 SELECT에서도 선언이 가능하다.

#### 인텐션잠금과 테이블잠금 : 인텐션 잠금은 특정 레코드의 데이터만 잠금을 하지만, 테이블 잠금은 히스토리 포함한 전체의 데이터를 잠금시킨다. 그래서 인텐션 잠금은 배타적이여도 서로 범위가 다르면 충돌하지 않지만, 베타적 테이블 잠금은 잠금이 아니여도 읽을 수가 없다.

#### DBMS에서 잠금의 대기시간을 조절할 수 있다.

<br/>

### 7.4. 고립성

#### 고립성이란 앞서 말한대로 트랜잭션끼리 영향을 최소화 하기 위한 성질인데, 정도에따라 READ_UNCOMMITED, READ_COMMITED, REPEATABLE-READ, SERIALIZABLE 순으로 있다.

#### 1.READ_UNCOMMITED : 가장 엄격하지않은 고립성으로써, 한 세션에서 잠금중이고 다른 세션들에서 읽을때 변경됬지만 커밋되지않은 데이터를 히스토리 로그에서 읽어옵니다. 이를 Dirty-Read라고 부릅니다. 커밋후에는 테이블에서 직접 읽어옵니다.

#### 2.READ_COMMITED : 그 담으로 엄격하지 않는 고립성으로써, 한 세션에서 잠금중 데이터 변경중 이더라도, 변경 데이터는 히스토리 로그에 기록해놓고 다른 세션들에서 읽을때는 테이블에서 직접 읽어오기때문에 서로 영향을 주지 않을 수 있다. 그러나 커밋 시점에 따라서 동일한 질의라도 결과물이 달라 질 수 있다.

#### 3. REPEATABLE-READ : 테이블에서 직접 겁근하는 명시적인 잠금으로 데이터를 읽어오지 않는 이상, 무조건 히스토리 로그에서 데이터를 읽어오기때문에 히스토리 로그 커밋 여부 상관없이 트랜잭션시 히스토리 로그 데이터만 읽어온다. 인텐션 락을 이용한 경우 예상치 못한 결과가 발생 할 수 있다.

#### 4. SERIALIZABE : 가장 엄격한 고립성 단계로서, 트랜잭션 내에서 변경 질의가 전달될 경우, 다른 명시적인 트랜잭션 내에서 락을 가진 트랜잭셔만 테이블의 데이터에 접근이 가능하다. 잠금방식이 아닌 질의일 경우에는 히스토리 로그를 통해 데이터를 가져온다.